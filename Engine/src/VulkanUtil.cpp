...
namespace Engine
{
	...
	void VulkanUtil::InitVulkan(const std::string& applicationName, GLFWwindow* window)
	{
	...
	void VulkanUtil::DrawFrame(const std::function<void(vk::raii::CommandBuffer&)>& recordUiPass)
	{
	...
	vk::raii::Device& VulkanUtil::GetDevice()
	{
		return Device;
	}

	vk::raii::PhysicalDevice& VulkanUtil::GetPhysicalDevice()
	{
	...
	void VulkanUtil::CreateInstance(const std::string& applicationName)
	{
	...
	void VulkanUtil::CreateSurface(GLFWwindow* window)
	{
	...
	vk::Extent2D VulkanUtil::ChooseSwapExtent(vk::SurfaceCapabilitiesKHR const& capabilities)
	{
	...
	std::vector<const char*> VulkanUtil::GetRequiredInstanceExtensions()
	{
	...
	bool VulkanUtil::IsDeviceSuitable(const vk::raii::PhysicalDevice& physicalDevice)
	{
