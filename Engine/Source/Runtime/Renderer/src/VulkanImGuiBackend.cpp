...
namespace Nyx
{
	VulkanImGuiBackend::VulkanImGuiBackend(GLFWwindow* window, VulkanRenderer& vulkan)
		: Window(window)
		, Vulkan(&vulkan)
	{
	}
	...
	void VulkanImGuiBackend::Initialize(float width, float height)
	{
		if (bInitialized)
		{
			LOG_WARNING("VulkanImGuiBackend is already initialized!");
			return;
		}
	...
	void VulkanImGuiBackend::Shutdown()
	{
		if (!bInitialized)
		{
			LOG_WARNING("Attempted to shutdown an uninitialized VulkanImGuiBackend!");
			return;
		}
	...
	void VulkanImGuiBackend::SetStyle(uint32_t index)
	{
		ImGuiStyle& style = ImGui::GetStyle();
	...
	void VulkanImGuiBackend::HandleKey(int key, int scancode, int action, int mods)
	{
		ImGuiIO& io = ImGui::GetIO();
	...
	bool VulkanImGuiBackend::GetWantKeyCapture()
	{
		return ImGui::GetIO().WantCaptureKeyboard;
	}

	void VulkanImGuiBackend::CharPressed(uint32_t key)
	{
		ImGuiIO& io = ImGui::GetIO();
