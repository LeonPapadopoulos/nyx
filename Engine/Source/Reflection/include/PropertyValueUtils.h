...
namespace Nyx::Reflection
{
	...
	inline bool ArePropertyValuesEqual(const PropertyValue& a, const PropertyValue& b)
	{
		if (a.index() != b.index())
		{
			return false;
		...
		if (std::holds_alternative<glm::vec4>(a)) return NearlyEqual(std::get<glm::vec4>(a), std::get<glm::vec4>(b));
		if (std::holds_alternative<glm::quat>(a)) return NearlyEqual(std::get<glm::quat>(a), std::get<glm::quat>(b));
